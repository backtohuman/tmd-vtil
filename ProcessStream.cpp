...
std::uint32_t ProcessStream::read(void* buf, std::uint32_t size)
{
	if (!this->isOpen())
	...
	if (b_cache)
	{
		...
		if (readBytes == 0)
		{
			...
			readBytes = std::min<SIZE_T>(readBytes, size);
...
std::uint32_t ProcessStream::write(const void* buf, std::uint32_t size)
{
	if (!this->isOpen())
		throw std::runtime_error("process is not open");
